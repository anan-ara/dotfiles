...
-- Not needed since we have a statusline
vim.opt.showmode = false

-- Set completeopt to have a better completion experience
vim.opt.completeopt = 'menuone,noselect'

-- Treat underscore as not part of a word for vim movements
...
vim.keymap.set('n', '<Enter>', '<cmd>nohlsearch<cr>', { silent = true })

-- Remap for dealing with word wrap
vim.keymap.set({ "n", "x" }, "j", "v:count == 0 ? 'gj' : 'j'", { expr = true, silent = true })
vim.keymap.set({ "n", "x" }, "k", "v:count == 0 ? 'gk' : 'k'", { expr = true, silent = true })

-- Easier to reach for quick switches
vim.keymap.set("n", "<C-s>", "<C-^>", { silent = true })
...
-- git-blame
vim.keymap.set("n", "<leader>gb", "<cmd>GitBlameToggle<cr>", { desc = "Open File Tree" })
...
vim.keymap.set('n', '<leader>p', require('telescope.builtin').live_grep, { desc = 'Search Git Files' })

vim.keymap.set('n', '<leader>ff', require('telescope.builtin').git_files, { desc = '[F]ind [F]iles (Git Only)' })
